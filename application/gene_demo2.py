...
from misc_util import positive_effect, summarize_ate_samples, plot_ate_samples
...
def run_example(cause_gene, target_gene):
    ...
    ate_summary_pm, ate_summary_np, ate_summary_gnpp, summary_gbf = subsample_ate(
        a, testX_hat, testX, y, positive_effect, n_post, start_size=50, n_SS=10
    )
    ...
    for summary, label in zip(summaries, labels):
        summarize_ate_samples(summary, label, cause_gene, summary = 'positive')
    plot_ate_samples(*summaries, cause_gene, target_gene, summary='positive')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'figures/gbf_{cause_gene}.pdf', dpi=300)
    plt.show()

if __name__ == "__main__":
    run_example(cause_gene = 'TCF7', target_gene = 'SELL')